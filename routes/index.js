...
var patentctg = require("./model/PatentCategory");
...
router.post("/getFinalPrice", async (req, res) => {
  let patenttitle = req.body.pattitle;
  console.log(patenttitle);
  await auction
    .find({ title: patenttitle })
    .exec()
    .then((response) => {
      if (response) {
        console.log(response);
        res.status(200).json({ price: response });
      } else {
        res.status(401).json({ status: failed });
      }
    });
...
router.post("/AddPatentcategory", async (req, res, next) => {
  const PatentCtg = new patentctg({
    category: req.body.category,
  });

  console.log(req.body.type);
  var response = await PatentCtg.save()
    .then((response) => {
      res.json({
        message: response,
      });
...
router.get("/ShowPatentcategory", async (req, res) => {
  await patentctg
    .find()
    .exec()
    .then((response) => {
...
router.post("/searchpatent", async (req, res) => {
  let search = req.body.search;
  await patentideas
    .find({ classification: new RegExp("^" + search + "$", "i") })
    .exec()
    .then((response) => {
      if (response) {
        res.status(200).json({ patent: response });
      } else {
        res.status(401).json({ status: "failed" });
      }
    });
